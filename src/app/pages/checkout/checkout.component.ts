...
export class CheckoutComponent implements OnInit, OnDestroy {
    ...
    private async initiateRazorpayPayment(response: any): Promise<void> {
        const options: any = {
            key: response.key,
            ...
            handler: async (razorpayResponse: any) => {
                ...
                try {
                    const payload = await firstValueFrom(this.prepareOrderPayload());
                    if (!payload) {
                        this.displayMessage(
                    ...
                    if (verificationResponse.success) {
                        this.cartService.clearCart();
                        this.showLoader = false; // hide loader just before navigating
