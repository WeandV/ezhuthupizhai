...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  placeOrder(): void {
    ...
    if (this.selectedPaymentMethod === 'COD') {
      ...
      this.placeOrderRazorpay();
    }
  }

  private placeOrderCOD(): void {
  ...
  private placeOrderRazorpay(): void {
    const payload = this.prepareOrderPayload();
    if (!payload) {
      return;
    }
      ...
      .subscribe({
        next: (response: any) => {
          if (response.success) {
            ...
            this.displayMessage(
              response.message || 'Failed to create Razorpay order. Please try again.',
              'error'
            );
          }
        },
  ...
  private initiateRazorpayPayment(response: any): void {
    const options: any = {
      ...
      handler: (razorpayResponse: any) => {
        // Send the complete payload including payment details for final verification and order creation
        const payload = this.prepareOrderPayload();
        if (!payload) {
          this.displayMessage(
            'Order details could not be prepared. Please contact support.',
            'error'
          );
          return;
        }
        ...
        this.http
          .post(environment.apiUrl + 'checkout/verify_payment', finalPayload)
          .subscribe({
            next: (verificationResponse: any) => {
              if (verificationResponse.success) {
                this.displayMessage('Payment verified and order complete!', 'success');
                this.cartService.clearCart();
                this.router.navigate(['/order-confirmation', verificationResponse.order_id]);
              } else {
                this.displayMessage(
  ...
  private prepareOrderPayload(): any | null {
    ...
    // Use combineLatest to get the latest values from observables
    let combinedPayload: any | null = null;
    combineLatest([
